...
$event = \mod_jupyter\event\course_module_instance_list_viewed::create(['context' => $modulecontext]);
...
if (empty($jupyters)) {
    notice(get_string('no$jupyterinstances', 'mod_jupyter'), new moodle_url('/course/view.php', ['id' => $course->id]));
}
...
foreach ($jupyters as $jupyter) {
    if (!$jupyter->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/jupyter/view.php', ['id' => $jupyter->coursemodule]),
    ...
    if ($course->format == 'weeks' || $course->format == 'topics') {
        $table->data[] = [$jupyter->section, $link];
    } else {
        $table->data[] = [$link];
    }
}
